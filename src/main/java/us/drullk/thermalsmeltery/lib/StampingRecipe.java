...
public class StampingRecipe {
	...
	public StampingRecipe(LiquidCasting tableCasting, CastingRecipe recipe) {
		...
		cast = recipe.cast.copy();
		coolTime = recipe.coolTime;

		float value = 0;

		try {
			metal = tableCasting.getCastingRecipe(recipe.castingMetal, new ItemStack(TinkerSmeltery.metalPattern, 1, 0)).getResult();
		...
		if (value > 0.1) {
			...
			if (secondaryResult.stackSize <= 0) {
	...
	public boolean matches(ItemStack metal, ItemStack inputCast) {
		return MachineRecipeRegistry.isOreDictMatch(metal, metal) && metal.stackSize >= metal.stackSize && ((cast != null && cast.getItemDamage() == Short.MAX_VALUE && inputCast.getItem() == cast.getItem()) || ItemStack.areItemStacksEqual(cast, inputCast));
	}
